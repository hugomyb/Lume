...
export const ja: Dict = {
  ...
  "cmd.noCli":
    "PATH に <code>{cmd}</code> が見つかりません — 設定 › AI でプロバイダーを設定してください。",
  "cmd.noProvider":
    "AI プロバイダーが設定されていません — 設定 › AI で設定してください。",
  "cmd.cancel": "キャンセル",
  ...
  "blocks.aiThinking": "{provider} が考えています…",
  "blocks.aiError": "エラー",
  "blocks.aiClose": "閉じる",
  ...
  "blocks.helpKeys":
    'クリック → ターミナルのプロンプトへスクロール。<br/>右クリック → コピー、挿入、説明。<br/><kbd>Ctrl</kbd>+<kbd>↑</kbd>/<kbd>↓</kbd> → 移動、<kbd>↩</kbd> でターミナルに挿入。<br/><span style="color: var(--accent)">✨</span> → AI による説明。',
  ...
  "blocks.cancel": "キャンセル",
  "blocks.aiFixError": "{provider} でこのエラーを修正",
  ...
  "ai.command": "コマンド",
  "ai.args": "引数",
  "ai.keyEnv": "API キーの環境変数",
  "ai.claudeNote": "<code>claude</code> CLI（Claude Code）を使用します。認証するには一度 <code>claude login</code> を実行してください。",
  "ai.codexNote": "<code>codex</code> CLI（OpenAI）を使用します。<code>codex login</code> で認証するか、上で API キーを設定してください（<code>OPENAI_API_KEY</code> として注入されます）。",
  "ai.customNote": "prompt を受け取り、回答を stdout に出力する任意の CLI。prompt を入れる位置に引数内で <code>{prompt}</code> を指定してください。",
