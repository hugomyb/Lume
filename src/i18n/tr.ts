...
export const tr: Dict = {
  ...
  "cmd.noCli":
    "<code>{cmd}</code> PATH'te bulunamadı — Ayarlar › YZ bölümünden bir sağlayıcı yapılandırın.",
  "cmd.noProvider":
    "Yapılandırılmış YZ sağlayıcısı yok — Ayarlar › YZ bölümünden bir tane ayarlayın.",
  "cmd.cancel": "İptal",
  ...
  "blocks.aiThinking": "{provider} düşünüyor…",
  "blocks.aiError": "Hata",
  ...
  "blocks.helpKeys":
    'Tıkla → terminaldeki prompt\'a kaydır.<br/>Sağ tıkla → kopyala, ekle, açıkla.<br/><kbd>Ctrl</kbd>+<kbd>↑</kbd>/<kbd>↓</kbd> → gez, terminale eklemek için <kbd>↩</kbd>.<br/><span style="color: var(--accent)">✨</span> → YZ açıklaması.',
  ...
  "blocks.cancel": "İptal",
  "blocks.aiFixError": "Bu hatayı {provider} ile düzelt",
  ...
  "ai.command": "Komut",
  "ai.args": "Argümanlar",
  "ai.keyEnv": "API anahtarı ortam değişkeni",
  "ai.claudeNote": "<code>claude</code> CLI'sini (Claude Code) kullanır. Kimlik doğrulamak için bir kez <code>claude login</code> çalıştırın.",
  "ai.codexNote": "<code>codex</code> CLI'sini (OpenAI) kullanır. <code>codex login</code> ile kimlik doğrulayın veya yukarıdan bir API anahtarı ayarlayın (<code>OPENAI_API_KEY</code> olarak enjekte edilir).",
  "ai.customNote": "Bir prompt alıp yanıtı stdout'a yazdıran herhangi bir CLI. Prompt'un gitmesi gereken yere argümanlar içinde <code>{prompt}</code> koyun.",
